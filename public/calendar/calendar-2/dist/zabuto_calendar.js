	...
	(function ($, window, document, undefined) {
		...
		$.extend(ZabutoCalendar.prototype, {
			...
			_renderNavigation: function (year, month) {
				...
				if (null === toPrev && null === toNext) {
					nav.append(header.attr('colspan', 3));
				} else {
			...
			_renderDaysInMonth: function (year, month) {
				...
				for (var wk = 1; wk <= weeks; wk++) {
                  ...
                  row.append($('<td class="text-center available"> <i class="fa fa-check  text-success"> </i></td>'));
				  row.append($('<td class="text-center not-available"> <i class="fa fa-close  text-danger"> </i></td>'));
